#!/usr/bin/env python3
"""
Enhanced Word-Level JSON to SRT Converter with Intelligent Timing Corrections
============================================================================

This enhanced version includes intelligent timing corrections for words with
anomalous durations based on position (first/last) and word characteristics.
"""
...
class WordLevelToSRTConverter:
    """Converts word-level JSON to SRT with intelligent timing corrections"""

    def __init__(self, pause_threshold: float = 3.0, buffer_before: float = 0.2,
                 buffer_after: float = 1.5, max_chars_per_line: int = 40,
                 max_lines: int = 2, overflow_tolerance: int = 4, add_hyphens: bool = True,
                 max_subtitle_duration: float = 50.0, apply_timing_corrections: bool = True,
    ...
    def detect_segment_breaks(self, words: List[Word]) -> List[int]:
        ...
        for i in range(1, len(words)):
            ...
            # Speaker change (only if break_on_speaker_change is enabled)
            if self.break_on_speaker_change and prev_word.speaker != current_word.speaker:
                breaks.append(i)
    ...
    def create_segments(self, words: List[Word]) -> List[SubtitleSegment]:
        ...
        for i in range(len(all_breaks) - 1):
            ...
            for word_group in word_groups:
                ...
                if segment.end_time - segment.start_time > self.max_subtitle_duration:
                    # Calculate new end time
                    new_end_time = segment.start_time + self.max_subtitle_duration
    ...
    def apply_timing_buffers(self, segments: List[SubtitleSegment]) -> List[SubtitleSegment]:
        """Apply timing buffers and resolve overlaps properly"""
        if not segments:
            return []
        ...
        # First pass: Apply buffers to all segments
        for segment in segments:
            segment.start_time = max(0, segment.start_time - self.buffer_before)
            segment.end_time += self.buffer_after

        # Second pass: Resolve overlaps after buffers are applied
        for i in range(len(segments) - 1):
            current_segment = segments[i]
            next_segment = segments[i + 1]

            if current_segment.end_time > next_segment.start_time:
                # Calculate clean midpoint split
                overlap_amount = current_segment.end_time - next_segment.start_time
                midpoint = next_segment.start_time + (overlap_amount / 2)

                # Set end time of current and start time of next with small gap
                current_segment.end_time = midpoint - 0.01
    ...
    def add_continuation_hyphens(self, text: str) -> str:
        """Add continuation hyphens to text based on smart rules

        IMPORTANT: Within a single subtitle, lines should NOT have continuation hyphens.
        Continuation hyphens are only for indicating continuation BETWEEN subtitles.
        """
        if not self.add_hyphens or not text:
            return text
        ...
        for i, line in enumerate(lines):
            ...
            # Fix broken word hyphens (e.g., "program-" should be "program -")
            if line.endswith('-') and not line.endswith(' -') and not line.endswith('--'):
                # This looks like a broken word hyphen, fix it
                line = line[:-1] + ' -'

            # WITHIN A SINGLE SUBTITLE: No continuation hyphens between lines
            # The lines within a subtitle are part of the same logical unit
            # Only the LAST line of a subtitle might end with " -" to indicate
            # continuation to the NEXT subtitle, but this is handled at the
            # subtitle level, not here.
    ...
    def process_file(self, input_path: str, output_dir: str = None) -> tuple:
        ...
        print("Validating word timing consistency...")

        print("Creating subtitle segments...")
        segments = self.create_segments(words)

        print("Applying timing buffers and resolving overlaps...")
        segments = self.apply_timing_buffers(segments)
    ...
    def print_stats(self):
        ...
        print(f"Timeline overlaps resolved: {self.stats.overlaps_resolved}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Enhanced Word-Level JSON to SRT Converter with Intelligent Timing Corrections",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
EXAMPLES:
    # Basic usage with timing corrections (default)
    python word_level_to_srt_with_corrections.py transcript.json

    # Disable timing corrections
    python word_level_to_srt_with_corrections.py transcript.json --no-timing-corrections

    # Custom correction threshold (only correct words longer than 5s)
    python word_level_to_srt_with_corrections.py transcript.json --correction-threshold 5.0

    # Specify output directory
    python word_level_to_srt_with_corrections.py transcript.json --output-dir ./corrected_subs/

TIMING CORRECTIONS:
    This enhanced version intelligently corrects anomalous word durations:
    - First words: Removes pre-speech silence/pauses
    - Last words: Removes post-speech silence/pauses
    - Preserves natural speech timing while fixing processing artifacts
    - Based on word length and position analysis

ORPHAN PREVENTION:
    Improves subtitle readability by preventing orphan words:
    - Detects sentence starts at the end of subtitles
    - Moves short sentence fragments (up to 15 chars) to next subtitle
    - Ensures complete thoughts remain together
    - Enhances human readability and comprehension
        """
    )
    ...
    parser.add_argument('--buffer-before', type=float, default=0.2, help='Time buffer before speech in seconds (default: 0.2)')
    parser.add_argument('--buffer-after', type=float, default=1.5, help='Time buffer after speech in seconds (default: 1.5)')
    ...
    parser.add_argument('--max-subtitle-duration', type=float, default=50.0, help='Maximum subtitle duration in seconds (default: 50.0)')
    ...
    parser.add_argument('--no-break-on-speaker-change', action='store_false', dest='break_on_speaker_change', help='Disable breaking on speaker changes (useful for broken speaker detection)')
    ...
    print("Starting enhanced word-level to SRT conversion...")
    print(f"Input: {args.input_json}")
    print(f"Output directory: {args.output_dir or 'same as input'}")

    print("Configuration:")
    print(f"   Pause threshold: {args.pause_threshold}s")
    print(f"   Buffer: {args.buffer_before}s before, {args.buffer_after}s after")
    ...
    print(f"   Hyphen space reserved: 2 chars for continuation hyphens")
