...
MIN_REASONABLE_DURATION = 0.05
MAX_REASONABLE_DURATION = 3.0
BASE_SYLLABLE_DURATION = 0.2
SOFT_PUNCT_ADJUSTMENT = 0.05
HARD_PUNCT_ADJUSTMENT = 0.1
...
class WordLevelToSRTConverter:
    ...
    @staticmethod
    def _get_punctuation_type(text: str) -> str:
        """Classify punctuation type of text"""
        text = text.strip()
        if text.endswith(HARD_PUNCTUATION):
            return 'hard'
        elif text.endswith(SOFT_PUNCTUATION):
            return 'soft'
        return 'none'

    @staticmethod
    def print_duration_table(duration_stats: Dict[int, Dict[str, float]]) -> None:
        ...
        for vowel_count in sorted(duration_stats.keys()):
    ...
    def build_duration_statistics(self, words: List[Word]) -> Dict[int, Dict[str, float]]:
        ...
        for word in words:
            ...
            punct_type = self._get_punctuation_type(word.text)
        ...
        for vowel_count, punct_groups in duration_groups.items():
            ...
            for punct_type, durations in punct_groups.items():
                stats[vowel_count][punct_type] = statistics.median(durations) if durations else None

        return stats

    def estimate_typical_duration(self, word: str, duration_stats: Optional[Dict[int, Dict[str, float]]] = None) -> float:
        ...
        punct_type = self._get_punctuation_type(word)
        ...
        if duration_stats and vowel_count in duration_stats:
            ...
            # Fallback to 'none' with punctuation adjustment
            if punct_type != 'none':
                avg_duration = duration_stats[vowel_count].get('none')
                if avg_duration is not None:
                    adjustment = SOFT_PUNCT_ADJUSTMENT if punct_type == 'soft' else HARD_PUNCT_ADJUSTMENT
                    return avg_duration + adjustment

        # Fallback to heuristic: 0.2s per syllable/vowel group as base
        base_duration = vowel_count * BASE_SYLLABLE_DURATION + 0.1

        # Punctuation adjustments
        if punct_type == 'soft':
            base_duration += SOFT_PUNCT_ADJUSTMENT
        elif punct_type == 'hard':
            base_duration += HARD_PUNCT_ADJUSTMENT

        return max(0.1, base_duration)

    def apply_intelligent_timing_corrections(self, words: List[Word], duration_stats: Optional[Dict[int, Dict[str, float]]] = None) -> List[Word]:
        ...
        # POST-CORRECTION: Fix any overlaps created by timing corrections
        # After all corrections, ensure no word overlaps with the previous word
        for i in range(1, len(corrected_words)):
            ...
            if curr_word.start < prev_word.end:
                # Fix: Move current word's start to after previous word's end with safety gap
                curr_word.start = prev_word.end + self.safety_gap

                # Ensure current word still has minimum duration
    ...
    def load_json_data(self, file_path: str) -> List[Word]:
        ...
        for segment in segments:
            ...
            for word_data in segment_words_list:
                ...
                is_ellipsis = word_text in ELLIPSIS_VARIANTS
        ...
        # GLOBAL OVERLAP FIX: Ensure no word overlaps with previous word across all segments
        # This catches overlaps that occur between segments after corrections
        # Also ensures minimum 10ms gap (no touching)
        for i in range(1, len(words)):
            ...
            if gap < self.safety_gap:  # Less than safety gap (includes overlaps and touching)
                if gap < 0:
                    # Overlap: move current word forward
                    curr_word.start = prev_word.end + self.safety_gap
                    self.stats.overlaps_resolved += 1
                else:
                    # Touching or too-small gap: move previous end back
                    prev_word.end = curr_word.start - self.safety_gap

                    # Ensure previous word has minimum duration
                    if prev_word.end < prev_word.start + MIN_WORD_DURATION:
                        # Not enough room, move current word forward instead
                        curr_word.start = prev_word.start + MIN_WORD_DURATION + self.safety_gap

                    self.stats.overlaps_resolved += 1

                # Ensure current word has minimum duration
                if curr_word.end < curr_word.start + MIN_WORD_DURATION:
                    curr_word.end = curr_word.start + MIN_WORD_DURATION
    ...
    @staticmethod
    def calculate_text_length(words: List[Word]) -> int:
        """Calculate total character length of words when joined"""
        return len(" ".join(word.text for word in words)) if words else 0
    ...
    def check_for_orphans_in_wrapped_text(self, words: List[Word]) -> Optional[int]:
        ...
        # Check 1: Entire last line is an orphan sentence start
        ...
        # Check 2: Trailing orphan words at END of last line
        # Look for 1-2 words stranded at end like "...text, spør -"
        if len(last_line_words) >= 2:
            # Check last 1-2 words of the last line
            for trailing_word_count in [1, 2]:
                trailing_words = last_line_words[-trailing_word_count:]
                trailing_text = " ".join(trailing_words)

                # If these trailing words are short (orphan-like)
                if len(trailing_text) <= self.orphan_move_threshold:
                    # Calculate the split index (move these words to next subtitle)
                    split_index = len(words) - trailing_word_count

                    if split_index > 0:
                        # Make sure remaining text is substantial enough
                        remaining_words = words[:split_index]
                        remaining_lines = self.wrap_text(remaining_words)

                        # Only split if remaining text still fills at least 1 decent line
                        if remaining_lines and len(remaining_lines[-1]) > self.orphan_move_threshold:
                            self.stats.orphan_breaks_prevented += 1
                            return split_index
    ...
    def merge_leading_orphans(self, segments: List[SubtitleSegment]) -> List[SubtitleSegment]:
        """Merge short orphan words at the start of segments back to the previous segment

        This fixes cases where a segment break creates a new segment starting with
        just 1-2 short words (e.g., "- program.")

        IMPORTANT: Only merges if there's NO significant pause between segments.
        If there's a natural speech pause (>pause_threshold), keep the break even if it creates an orphan.
        This respects the speaker's natural rhythm.

        After merging, re-split any oversized segments to maintain line/duration limits.
        """
        ...
        while i < len(segments):
            ...
            # Check if this segment starts with orphan words
            if i > 0 and len(current_segment.words) <= 3:
                # Calculate text length of first few words
                first_words_text = ' '.join(word.text for word in current_segment.words[:2])
                first_words_length = len(first_words_text)

                # If very short (orphan), check if we should merge back to previous segment
                if first_words_length <= self.orphan_move_threshold:
                    ...
                    if prev_segment.speaker == current_segment.speaker:
                        ...
                        # If there's a natural pause (>pause_threshold), DON'T merge - respect speaker's rhythm
                        gap = current_segment.words[0].start - prev_segment.words[-1].end

                        if gap <= self.pause_threshold:
                            # No significant pause - this orphan was created by technical splitting
                            # Safe to merge back
                            merged_words = prev_segment.words + current_segment.words

                            # Remove the previous segment since we'll replace it
                            merged_segments.pop()

                            # Re-split the merged words in case they're now too long
    ...
    def create_segments(self, words: List[Word]) -> List[SubtitleSegment]:
        ...
        # Post-process: merge orphan words at segment starts back to previous segment
        segments = self.merge_leading_orphans(segments)

        return segments

    def _apply_reading_buffers(self, segments: List[SubtitleSegment]) -> None:
        """Apply reading time buffers to segments"""
        for i in range(len(segments) - 1):
            ...
            # For large pauses, preserve silence; for small gaps, add reading time
            if gap > self.pause_threshold:
                # Large pause - add minimal buffer, preserve most of the silence
                max_trailing_buffer = min(DEFAULT_READING_BUFFER, gap / 2)
                max_leading_buffer = min(DEFAULT_READING_BUFFER, gap / 2)

                # Ensure safety gap is preserved
                ...
                # Small gap - distribute available space for reading time
                # But cap at DEFAULT_READING_BUFFER to avoid excessive extension
    ...
    def _apply_duration_caps(self, segments: List[SubtitleSegment]) -> None:
        """Apply maximum duration caps to segments"""
        for seg in segments:
    ...
    def _ensure_safety_gaps(self, segments: List[SubtitleSegment]) -> None:
        """Ensure minimum safety gap between all segments"""
        for i in range(len(segments) - 1):
            current_segment = segments[i]
            next_segment = segments[i + 1]
            ...
            if gap < self.safety_gap:
                # Prefer moving end time back to preserve start times
                current_segment.end_time = next_segment.start_time - self.safety_gap

                # Ensure segment still has reasonable duration
                if current_segment.end_time < current_segment.start_time + 0.1:
                    # Not enough room, move next segment forward instead
    ...
    def apply_intelligent_buffers_and_caps(self, segments: List[SubtitleSegment]) -> List[SubtitleSegment]:
        """Applies reading time buffers and duration caps to segments.

        Buffers provide extra reading time by keeping subtitles visible slightly longer
        after speech ends. For large pauses (>pause_threshold), we preserve the silence.
        """
        self._apply_reading_buffers(segments)
        self._apply_duration_caps(segments)
        self._ensure_safety_gaps(segments)
        return segments
    ...
    def ends_with_hard_punctuation(text: str) -> bool:
        """Check if text ends with HARD punctuation (. ! ?) - Complete sentence boundary"""
        return text.strip().endswith(HARD_PUNCTUATION) if text.strip() else False

    @staticmethod
    def ends_with_soft_punctuation(text: str) -> bool:
        """Check if text ends with SOFT punctuation (, : ;) - Continuation implied"""
        return text.strip().endswith(SOFT_PUNCTUATION) if text.strip() else False

    @staticmethod
    def get_last_word(segment: SubtitleSegment) -> str:
        """Get the last word text from a segment"""
        return segment.words[-1].text.strip() if segment.words else ""

    @staticmethod
    def get_first_word(segment: SubtitleSegment) -> str:
        """Get the first word text from a segment"""
        return segment.words[0].text.strip() if segment.words else ""
    ...
    def apply_subtitle_continuation_hyphens(self, text: str, subtitle_index: int, all_segments: List[SubtitleSegment]) -> str:
        ...
        NOTE: pause_threshold is ONLY for breaking subtitles, NOT for hyphen decisions.
        Hyphens bind sentence continuations together regardless of gap length or speaker changes.
        """
    ...
    def print_stats(self):
        ...
        if self.apply_timing_corrections:
            ...
            print(f"  - Overlaps resolved: {self.stats.overlaps_resolved}")
